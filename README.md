...
Клон берётся на всю историю (до 2000 коммитов) — по ней рисуется дерево пути
создания репозитория. Если пачка не скачалась, вторая попытка берёт окно в 90 дней,
и дерево честно помечается обрезанным.
