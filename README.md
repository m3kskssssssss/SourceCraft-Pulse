...
|---|---:|---|
| **Безопасность** | 20 % | только открытые находки [SourceCraft AppSec](https://appsec.sourcecraft.tech): critical, high, medium и секреты. AppSec отдаёт результаты лишь участникам репозитория, поэтому у публичных репозиториев категория — «нет данных» у всех одинаково; приватный считается по токену владельца |
