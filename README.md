...
| **Безопасность** | только по данным SourceCraft AppSec: critical, high и medium уязвимости, lock-файлы, свежесть зависимостей, Dependabot или Renovate. Пока AppSec недоступен в публичном API, категория показывает «Нет данных» и исключается из итога. Проверка зависимостей по открытой базе [OSV.dev](https://osv.dev) выводится на странице анализа как справка и на балл не влияет |
...
без исправления (по данным AppSec), −8 за отсутствие LICENSE.
...
и историю мы читаем из клона. Категория «Безопасность» по ТЗ считается только по данным AppSec:
пока их нет, она показывает «Нет данных» и исключается из итогового балла (веса остальных
категорий нормируются). Собственным сканированием мы её не подменяем: известные уязвимости
