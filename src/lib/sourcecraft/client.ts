...
export class SourcecraftClient {
  ...
  /**
   * Discover: публичный каталог SourceCraft целиком (около 27 тысяч записей).
   * Обходить только сортировкой по `created_at`: при сортировке по рейтингу
   * страницы пропускают и повторяют записи. См. lib/catalog.ts.
   */
