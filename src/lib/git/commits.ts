...
export async function readCloneCommits(
  repo: RepoClone,
  limit: number,
  /** Откуда начинать. По умолчанию — HEAD; для ветки передаётся её верхушка. */
  ref: string = repo.headOid,
): Promise<RawCommit[]> {
  const log = await git.log({ fs, dir: repo.dir, ref, depth: limit, cache: repo.cache });
