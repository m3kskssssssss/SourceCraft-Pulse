...
// ---------- 7. Оценка: шесть колец-категорий и итог ----------
...
const CATEGORIES = [
  ...
  { label: 'ci/cd', value: 0.8 },
  { label: 'задачи', value: 0.66 },
];
...
function ScoreScene({ t }: { t: number }) {
  ...
  return (
    <g>
      ...
      {CATEGORIES.map((c, i) => {
        // Шаг 5: шесть колец укладываются в высоту сцены.
        const r = 26 + i * 5;
        const k = easeOut(prog(t, i * 0.3, 1.6 + i * 0.3));
...
export const PROCESS_STEPS: ProcessStep[] = [
  ...
  },
  {
    ...
    text: 'Балл безопасности — только по находкам SourceCraft AppSec, которые платформа отдаёт владельцу по его токену. Для справки сверяем lock-файлы с открытой базой OSV.dev и ищем в истории строки, похожие на ключи, — на балл это не влияет.',
    Scene: SecurityScene,
  },
  {
    key: 'ai',
  ...
  },
  {
    ...
    text: 'Шесть категорий ТЗ — безопасность и код по 20 %, активность, документация, CI/CD и задачи по 15 % — дают итог 0–100. Категории без данных не штрафуют, их вес делится между остальными. Затем подбираем рекомендации, которые дают больше всего баллов за единицу усилий.',
    Scene: ScoreScene,
  },
];
